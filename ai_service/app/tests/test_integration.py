...
async def test_find_related_endpoint():
    ...
    # Sample regulations (categories must match the database enum values)
    regulations = [
        {
            "id": 1,
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            response = await client.post(
                f"{base_url}/similarity/find-related", json=payload, timeout=30.0
            )
            ...
            if response.status_code == 200:
                ...
                for i, reg in enumerate(related, 1):
                    ...
                    print(
                        f"  {i}. Regulation ID {reg['regulation_id']}: {reg['title']}"
                    )
                    print(
                        f"     Score: {score:.4f} | Category: {reg.get('category', 'N/A')}"
                    )
    ...
    test_cases = [
        {
            ...
            "payload": {
                "case_text": "",
                "regulations": [{"id": 1, "title": "Test", "category": "labor_law"}],
            },
            "expected_status": 400,
        },
        {
            "name": "Empty regulations list",
            "payload": {"case_text": "Some case text", "regulations": []},
            "expected_status": 400,
        },
    ]
